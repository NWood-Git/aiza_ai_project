...
def schema(dbpath = 'mydb.db'):
        ...
        # Create Cohort Table
        DROP_SQL = "DROP TABLE IF EXISTS cohorts;"
        cur.execute(DROP_SQL)
        SQL = """CREATE TABLE cohorts (
...
def insert_cohorts(dbpath = 'mydb.db'):
        with sqlite3.connect(dbpath) as conn:
                cur = conn.cursor()
                SQL = "INSERT INTO cohorts(name) VALUES('griffin');"
                cur.execute(SQL)
                SQL = "INSERT INTO cohorts(name) VALUES('gizmo');"
                cur.execute(SQL)
